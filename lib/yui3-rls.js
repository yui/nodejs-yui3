...
exports.rls = function(YUI, config, fn) {
    ...
    var bootstrapCore = true;
    if (config.env) {
        bootstrapCore = config.env.every(function (env) {
            return env !== "yui"
        });
    }

    if (bootstrapCore) {
        files.push(YUI.GlobalConfig.base + 'yui/yui-min.js');
    }
    ...
    if (mods.indexOf('loader') === -1) {
        files.forEach(function(v, k) {
            if (v.indexOf('/loader/loader-min.js') !== -1) {
                files.splice(k, 1);
            }
        });
    }
    if (mods.indexOf('yui') === -1) {
        files.forEach(function(v, k) {
            if (v.indexOf('/yui/yui-min.js') !== -1) {
                files.splice(k, 1);
            }
        });
    }

    // If we didn't ask for YUI and it's the only file, we have a 404 situation
    // XXX This is hacky! We should add bootstrapCore's yui-min at the end
    // instead of above.
    if (bootstrapCore && files.length === 1 && config.m.join() !== "yui") {
        files = [];
    }
