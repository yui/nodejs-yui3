...
YUI.include = function(file, cb) {
    ...
    if (file.match(/^https?:\/\//)) {
        ...
        var urlInfo = url.parse(file, parseQueryString=false),
            ...
            host, req_url = urlInfo.pathname;
            
            if (urlInfo.port) {
                port = urlInfo.port;
            }
            host = http.createClient(port, urlInfo.hostname);
                ...
                if (err) {
                    ...
                    if (err.stack) {
                        A.each(err.stack.split('\n'), function(frame) {
                            Y.log(frame, 'error', 'get');
                        });
                    } else {
                        Y.log(getErrorMessage(err), 'error', 'get');
                        Y.log('In file: ' + getErrorFilename(err), 'error', 'get');
                        Y.log('On line: ' + getErrorLine(err), 'error', 'get');
                    }
                    Y.log('----------------------------------------------------------', 'error', 'get');
                }
                pass(cb);
